...
impl RunConfig {
    fn parser() -> OptionParser<Self> {
        let unpack = short('u').long("unpack").help("Unpack the game files to a directory").req_flag(AppBehaviour::Unpack);
        let pack = short('p').long("pack").help("Pack existing game files to an archive").req_flag(AppBehaviour::Pack);
        ...
        construct!(RunConfig { app_behaviour, input_path, output_path }).to_options()
    }
}
